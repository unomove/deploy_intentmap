...
class GPSLocalization(object):
    ...
    def __init__(self):
        ...
        self.odom_sub = rospy.Subscriber("/spot/odometry", Odometry, self.on_odometry)
        self.gps_sub = rospy.Subscriber("/mavros/global_position/global", NavSatFix, self.on_gps)
        ...
        self.gps_path_sub = rospy.Subscriber("/raw_gps_path", Path, self.cb_path)
        ...
        # Publish unfiltered odom path
        self.gps_path_pub = rospy.Publisher("fused_gps_path", Path, queue_size=1)
    ...
    def cb_path(self, msg):
        ...
        for point in msg.poses:
            measured_x = (point.pose.position.x - self.ref_lon) * (math.pi / 180.0) * RADIUS_EARTH * math.cos(self.ref_lat * math.pi / 180.0)
            measured_y = (point.pose.position.y - self.ref_lat) * (math.pi / 180.0) * RADIUS_EARTH
            gps_path.poses.append(self.pose_to_StampedPose([measured_x, measured_y, 0], gps_path.header.stamp, 'map'))
    ...
    def publish(self, time):
        ...
        if len(self.measurement_queue) > 0:
            ...
            for x in list(self.measurement_queue):
                map_pos = x["map"]
